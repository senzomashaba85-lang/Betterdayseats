...
const LIGHT = {
  ...
  text:    "#2c1a12",
  muted:   "#9a7060",
  grad:    "linear-gradient(135deg, #b8843a 0%, #b05a75 100%)",
  overlay: "linear-gradient(180deg, rgba(253,248,244,0) 0%, rgba(253,248,244,0.4) 40%, rgba(253,248,244,0.97) 100%)",
  inputBg: "#fff8f2",
...
const DEMO = [
  {
    ...
    comments:[{ id:1, text:"Made this last Sunday! Family loved it 😍", author_name:"Priya K.", created_at: new Date().toISOString() }],
  ...
  },
  {
    ...
    comments:[{ id:1, text:"Obsessed with this recipe! 🥑", author_name:"Lerato M.", created_at: new Date().toISOString() }],
  ...
  },
  {
    ...
    comments:[{ id:1, text:"Made these for my anniversary dinner, absolute hit! 💕", author_name:"Nomvula D.", created_at: new Date().toISOString() }],
  ...
  },
  {
    ...
    comments:[{ id:1, text:"Nothing beats a KZN bunny 🙌🏾", author_name:"Sipho N.", created_at: new Date().toISOString() }],
...
function Toast({ message, onClose, S }) {
  ...
  return (
    <div style={{ position:"fixed", bottom:"90px", left:"50%", transform:"translateX(-50%)",
      ...
      boxShadow:`0 8px 32px ${S.shadow}`, whiteSpace:"nowrap" }}>
      {message}
    </div>
  )
}
...
// ─── HERO BANNER ────────────────────────────────────────────────────────────
function HeroBanner({ recipe, onSignup, currentUser, onPost, S }) {
  if (!recipe) return null
  return (
    <div style={{ position:"relative", width:"100%", height:"480px", overflow:"hidden", borderBottom:`1px solid ${S.border}` }}>
      <img src={recipe.image_url} alt={recipe.title}
        style={{ width:"100%", height:"100%", objectFit:"cover", display:"block", filter:"brightness(0.6) saturate(1.2)" }} />
      ...
      <div className="bde-float" style={{ position:"absolute", top:"28px", left:"28px",
        ...
        background: S.mode === "dark" ? "rgba(9,6,10,0.6)" : "rgba(253,248,244,0.85)",
        backdropFilter:"blur(12px)", border:`1px solid ${S.gold}40`, borderRadius:"40px", padding:"8px 16px" }}>
      ...
      <div style={{ position:"absolute", bottom:"36px", left:"32px", right:"32px", maxWidth:"680px" }}>
        <div className="bde-hero-text">
          ...
          <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(28px,5vw,52px)",
            fontWeight:"900", color: S.mode === "dark" ? "#f5e6d0" : "#fff",
            lineHeight:"1.1", marginBottom:"14px", textShadow:"0 2px 20px rgba(0,0,0,0.5)" }}>
            {recipe.title}
          </h1>
        </div>
        <p className="bde-hero-text-delay" style={{ color: S.mode === "dark" ? "#c8b8c4" : "#f0e8e8",
          fontSize:"15px", lineHeight:"1.65", maxWidth:"520px", marginBottom:"24px",
        ...
        <div className="bde-hero-text-delay" style={{ display:"flex", alignItems:"center", gap:"16px", flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
            ...
            <span style={{ color:"#fff", fontSize:"13px", textShadow:"0 1px 4px rgba(0,0,0,0.5)" }}>
              by <strong>{recipe.author_name}</strong>
            </span>
            <span style={{ color:"rgba(255,255,255,0.7)", fontSize:"13px" }}>· {recipe.likes} likes</span>
          </div>
        </div>
      </div>
    </div>
  ...
  const addComment = async () => {
    ...
    if (!isDemo) {
      ...
      setComments(prev => [...prev, {
        ...
        author_name: currentUser.name, created_at: new Date().toISOString()
      }])
    }
    setNewComment("")
    setSubmitting(false)
    ...
    <article className="bde-card" style={{ backgroundColor:S.card, borderRadius:"20px",
      ...
      <div style={{ padding:"18px 18px 0 18px", flex:1 }}>
          ...
          <div style={{ display:"flex", gap:"6px", marginTop:"10px", flexWrap:"wrap" }}>
            ...
            {recipe.socials.twitter && (
              <a href={recipe.socials.twitter.startsWith('http') ? recipe.socials.twitter : `https://twitter.com/${recipe.socials.twitter.replace('@','')}`}
                target="_blank" rel="noopener noreferrer"
                style={{ color:"#1DA1F2", fontSize:"11px", textDecoration:"none", background:`${S.rose}18`, padding:"3px 10px", borderRadius:"12px", display:"inline-flex", alignItems:"center", gap:"4px" }}>
                <FaTwitter size={11} /> X
              </a>
            )}
            ...
            {recipe.socials.youtube && (
              <a href={recipe.socials.youtube} target="_blank" rel="noopener noreferrer"
                style={{ color:"#FF0000", fontSize:"11px", textDecoration:"none", background:`${S.rose}18`, padding:"3px 10px", borderRadius:"12px", display:"inline-flex", alignItems:"center", gap:"4px" }}>
        ...
        {recipe.ingredients?.length > 0 && (
          <div style={{ marginTop:"12px", padding:"10px 14px", background:S.card2,
            borderRadius:"10px", border:`1px solid ${S.border}` }}>
            <div style={{ color:S.gold, fontSize:"11px", fontWeight:"600", marginBottom:"6px", letterSpacing:"0.5px" }}>🛒 Ingredients</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:"4px" }}>
              {(showAllIngredients ? recipe.ingredients : recipe.ingredients.slice(0,5)).map((ing,i) => (
                <span key={i} style={{ background:`${S.gold}18`, color:S.champ,
              ...
              <div style={{ display:"flex", gap:"8px", marginTop:"10px" }}>
                <input value={newComment} onChange={e => setNewComment(e.target.value)}
                  ...
                  style={{ flex:1, background:S.card2, border:`1px solid ${S.border}`,
                    borderRadius:"40px", padding:"9px 16px", color:S.text, fontSize:"13px", outline:"none" }} />
                <button className="bde-btn-grad" onClick={addComment} disabled={submitting}
                  style={{ borderRadius:"40px", padding:"9px 18px", fontSize:"13px", background:S.grad, color:"#fff" }}>
                  {submitting ? "..." : "Post"}
                </button>
              </div>
            ) : (
  ...
  return (
    <div style={{ position:"fixed", inset:0, background: S.mode === "dark" ? "rgba(9,6,10,0.96)" : "rgba(60,30,20,0.7)",
      ...
      <div style={{ background:S.card, borderRadius:"24px", width:"100%", maxWidth:"560px",
        ...
        padding:"32px", position:"relative", boxShadow:`0 32px 80px ${S.shadow}` }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"24px" }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif", color:S.champ, fontSize:"22px", fontWeight:"700" }}>Share Your Recipe ✨</h2>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
            ...
            <label style={{ cursor:"pointer" }}>
              ...
              <div style={{ border:`2px dashed ${S.rose}60`, borderRadius:"14px", padding:"24px", textAlign:"center", background:`${S.rose}05` }}>
              ...
              <div style={{ position:"relative", borderRadius:"12px", overflow:"hidden" }}>
                ...
                <button type="button" onClick={() => { setMediaFile(null); setMediaPreview("") }}
                  style={{ position:"absolute", top:"8px", right:"8px", background:S.bg,
                    border:"none", color:"#ef4444", borderRadius:"50%", width:"28px", height:"28px", cursor:"pointer" }}>×</button>
              </div>
            )}
            <button type="submit" className="bde-btn-grad" disabled={uploading}
              style={{ padding:"14px", borderRadius:"12px", fontSize:"15px", marginTop:"4px",
                background:S.grad, color:"#fff", opacity: uploading ? 0.7 : 1 }}>
    ...
    try {
      if (isSignUp) {
        ...
        if (data.user) {
          ...
          onLogin({ id: data.user.id, email, name: name.trim() || email.split("@")[0],
            countryFlag: sel?.flag || "🌍", avatar })
          onClose()
        }
      } else {
    ...
    <div style={{ position:"fixed", inset:0, background: S.mode === "dark" ? "rgba(9,6,10,0.97)" : "rgba(60,30,20,0.7)",
      ...
      <div style={{ background:S.card, borderRadius:"28px", maxWidth:"400px", width:"100%",
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"12px" }}>
          ...
          <input type="email" placeholder="Email address *" value={email} onChange={e => setEmail(e.target.value)} required style={iStyle} />
          ...
          <button type="submit" className="bde-btn-grad" disabled={loading}
            style={{ padding:"14px", borderRadius:"12px", fontSize:"15px", marginTop:"4px",
              background:S.grad, color:"#fff", opacity: loading ? 0.7 : 1 }}>
      ...
      <div style={{ background:S.card, borderRadius:"24px", padding:"48px 36px",
        ...
        <button onClick={onBack}
          style={{ position:"absolute", top:"16px", left:"16px", background:"none", border:"none",
            color:S.muted, fontSize:"16px", cursor:"pointer", padding:"8px", borderRadius:"8px",
  ...
  return (
    <div style={{ position:"fixed", bottom:"64px", left:0, right:0, zIndex:500,
      ...
      <button className="bde-btn-grad" onClick={onAccept}
        style={{ padding:"7px 20px", borderRadius:"40px", fontSize:"12px", background:S.grad, color:"#fff" }}>Accept</button>
    </div>
  )
}
...
export default function App() {
  ...
  const [isDark, setIsDark] = useState(() => {
    const saved = localStorage.getItem("bde_theme")
  ...
  const toggleTheme = () => {
    setIsDark(prev => {
      const next = !prev
      localStorage.setItem("bde_theme", next ? "dark" : "light")
      return next
    })
  }
  ...
  useEffect(() => {
    const loadRecipes = async () => {
      ...
      if (!error && data) {
        const withComments = await Promise.all(data.map(async recipe => {
          const { data: comments } = await supabase.from("comments").select("*").eq("recipe_id", recipe.id).order("created_at", { ascending: true })
          return { ...recipe, comments: comments || [], liked: false, saved: false }
        }))
        setDbRecipes(withComments)
      }
      setLoadingDb(false)
    }
    loadRecipes()
  ...
  const handleCreate = r => { setDbRecipes(p => [r, ...p]); showToast("Recipe shared! ✨"); addNotif(`✨ "${r.title}" is now live!`) }

  const handleLike = async id => {
    const isDemo = String(id).startsWith("demo-")
    if (isDemo) { setRecipes(p => p.map(r => r.id === id ? { ...r, likes: r.liked ? r.likes-1 : r.likes+1, liked: !r.liked } : r)); return }
    const recipe = dbRecipes.find(r => r.id === id); if (!recipe || !currentUser) return
    if (recipe.liked) {
  ...
  const handleSave = id => {
    const isDemo = String(id).startsWith("demo-")
    if (isDemo) setRecipes(p => p.map(r => r.id === id ? { ...r, saved: !r.saved } : r))
    else setDbRecipes(p => p.map(r => r.id === id ? { ...r, saved: !r.saved } : r))
  }
  ...
  const pages = {
    restaurants: <StubPage icon="🏪" title="Restaurants" body="Discover the best restaurants near you." onBack={() => setCurrentPage("home")} S={S} />,
    jobs:        <StubPage icon="💼" title="Food Industry Jobs" body="Find your next role in hospitality & food." onBack={() => setCurrentPage("home")} S={S} />,
    equipment:   <StubPage icon="🛒" title="Kitchen Equipment" body="Shop tools used by top chefs." onBack={() => setCurrentPage("home")} S={S} />,
    business:    <StubPage icon="💡" title="Business Ideas" body="Start your food business with our guides." onBack={() => setCurrentPage("home")} S={S} />,
    ...
    donations:   <StubPage icon="❤️" title="Donate" body="Support food NGOs fighting hunger across Africa." onBack={() => setCurrentPage("home")} S={S} />,
    books:       <StubPage icon="📚" title="Cookbooks" body="Curated cookbooks from chefs around the world." onBack={() => setCurrentPage("home")} S={S} />,
  ...
  return (
    <div style={{ backgroundColor:S.bg, minHeight:"100vh", color:S.text, transition:"background 0.3s, color 0.3s" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
              ...
              <div style={{ position:"relative" }}>
                <input placeholder="Search recipes…" value={searchTerm} onChange={e => setSearchTerm(e.target.value)}
                  style={{ background:S.card2, border:`1px solid ${S.border}`, borderRadius:"40px",
                    padding:"9px 16px 9px 38px", color:S.text, width:"180px", fontSize:"13px", outline:"none" }} />
            ...
            {currentUser ? (
              <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                ...
                <button onClick={handleLogout}
                  style={{ background:"none", border:`1px solid ${S.border}`, color:S.muted,
                    borderRadius:"8px", padding:"6px 12px", cursor:"pointer", fontSize:"12px" }}>Out</button>
                {!isMobile && (
                  <button className="bde-btn-grad" onClick={() => setShowCreate(true)}
                    style={{ padding:"9px 20px", borderRadius:"40px", fontSize:"13px", background:S.grad, color:"#fff" }}>
                    ✨ Post
                  </button>
                )}
              </div>
              ...
              <button className="bde-btn-grad" onClick={() => setShowAuth(true)}
                style={{ padding:"9px 22px", borderRadius:"40px", fontSize:"13px", background:S.grad, color:"#fff" }}>
                Join Free
              </button>
            )}
          </div>
          ...
          <div ref={dropRef} style={{ position:"relative", display:"inline-block", zIndex:99999 }}>
            <button onClick={() => { setShowDrop(!showDrop); if (showNotifs) setShowNotifs(false) }}
              style={{ background: showDrop ? `${S.rose}18` : "none",
                ...
                padding:"12px 16px", cursor:"pointer", fontSize:"13px",
                display:"flex", alignItems:"center", gap:"6px",
                borderBottom: showDrop ? `2px solid ${S.rose}` : "2px solid transparent", transition:"all 0.2s" }}>
      ...
      {currentPage === "home" && (
        <>
          <HeroBanner recipe={featured} onSignup={() => setShowAuth(true)} currentUser={currentUser} onPost={() => setShowCreate(true)} S={S} />
              ...
              {categories.map(cat => (
                <button key={cat.id} onClick={() => setSelectedCat(cat.id)}
                  style={{
                    ...
                    color: selectedCat === cat.id ? "#fff" : S.muted, transition:"all 0.2s"
                  }}>
                  {cat.icon} {cat.name}
                </button>
              ))}
      ...
      {currentPage === "home" && (
        <footer style={{ textAlign:"center", padding:"32px 24px", borderTop:`1px solid ${S.border}`, paddingBottom:"80px" }}>
          ...
          <p style={{ color:S.muted, fontSize:"11px", marginBottom:"12px" }}>© 2026 BetterDays Agile Technologies Inc · South Africa 🇿🇦</p>
          <div style={{ display:"flex", justifyContent:"center", gap:"16px", flexWrap:"wrap" }}>
            {["about","faq","contact","privacy","terms"].map(p => (
              <button key={p} onClick={() => setCurrentPage(p)}
                style={{ background:"none", border:"none", color:S.muted, cursor:"pointer", fontSize:"12px", textTransform:"capitalize" }}>
                {p}
              </button>
            ))}
          </div>
      ...
      <div style={{ position:"fixed", bottom:0, left:0, right:0,
        ...
        <button className="bde-btn-grad" onClick={openPost}
          style={{ borderRadius:"50%", width:"48px", height:"48px", display:"flex", alignItems:"center",
            justifyContent:"center", fontSize:"20px", marginTop:"-10px",
            background:S.grad, color:"#fff", boxShadow:`0 4px 20px ${S.rose}50` }}>+</button>
        <button className="bde-icon-btn" onClick={() => setShowNotifs(!showNotifs)}
